local ani
local check = GetSongFrame() == "_normal" and true or false
...
local function UpdateSingleBPM(self)
	if isGamePlay() then
		...
		local bpm0 = SCREENMAN:GetTopScreen():GetTrueBPS(p0) * 60
		bpmDisplaySingle:settext( string.format("%03.0f",bpm0) )
		if check and GAMESTATE:GetNumPlayersEnabled() > 1 then
			...
			local bpm1 = SCREENMAN:GetTopScreen():GetTrueBPS(p1) * 60
			local bpm2 = SCREENMAN:GetTopScreen():GetTrueBPS(p2) * 60
			bpmDisplayVersusP1:settext( string.format("%03.0f",bpm1) )
			bpmDisplayVersusP2:settext( string.format("%03.0f",bpm2) )
