local offsetInfo = getenv("OffsetTableP1")
local offsetInfoAll = getenv("OffsetTableAllP1")
...
local max = StepsOrTrail:GetRadarValues(PLAYER_1):GetValue("RadarCategory_TapsAndHolds") or 0
local misses = 0
...
showOffset = showOffset and offsetInfo and #offsetInfo > 0 and (counter+misses) > 0 and getenv("EvalComboP1")
...
return Def.ActorFrame{
	...
	Def.ActorFrame{
		...
		OnCommand=function()
			if getenv("EvalComboP1") and not StepsUpdate[PLAYER_1] then
		...
		OffCommand=function() StepsUpdate[PLAYER_1] = false end,
		Name="JudgeFrames",
		Def.ActorFrame{
			Name="W0",
			Condition=faplus,
	...
	Def.BitmapText {
		Condition=getenv("EvalComboP1") and counter~=max and (counter+misses)~=max,
		File="_v 26px bold shadow",
		InitCommand=function(self)
			if STATSMAN:GetCurStageStats(PLAYER_1):GetPlayerStageStats(PLAYER_1):GetFailed() then
