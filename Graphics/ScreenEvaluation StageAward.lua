...
if award and not STATSMAN:GetCurStageStats():GetPlayerStageStats(player):GetFailed() then
	...
	t[#t+1] = LoadFont("_eurostile normal")..{
		Name="Text",
		InitCommand=function(self) self:zoom(WideScreenDiff()):halign(1):shadowlength(2):maxwidth(220):settext(THEME:GetString( "StageAward", ToEnumShortString(award) )) end
	}
end
