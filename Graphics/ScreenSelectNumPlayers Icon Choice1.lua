return Def.ActorFrame{
	...
	Def.BitmapText {
		...
		InitCommand=function(self) self:x(SCREEN_CENTER_X+100*((WideScreenDiff()+WideScreenSemiDiff())/2)):y(SCREEN_CENTER_Y+125+12*WideScreenSemiDiff()):zoom(0.68*WideScreenDiff_(16/10)):maxwidth(840*WideScreenDiff_(16/10)):shadowlength(0) end,
	...
	Def.ActorFrame{
		InitCommand=function(self) self:x(SCREEN_LEFT+111*WideScreenDiff()):y(SCREEN_CENTER_Y-95):horizalign(left) end,
		Def.Sprite {
			Texture = THEME:GetPathG("_join","icons/choice1"),
		...
		Def.Sprite {
			Texture = THEME:GetPathG("_join","icons/single_icon"),
