...
return Def.ActorFrame{
	LoadFont("_r bold glow 30px")..{
		...
		InitCommand=function(self) self:x(SCREEN_CENTER_X+100):y(SCREEN_CENTER_Y+138):zoom(.68):maxwidth(840):horizalign(center):shadowlength(0) end;
	...
	Def.ActorFrame{
		InitCommand=function(self) self:x(SCREEN_LEFT+65):y(SCREEN_CENTER_Y-95):horizalign(left):ZoomToWidth(150):ZoomToHeight(110) end;
		LoadActor(THEME:GetPathG("_join","icons/choice1"))..{
		...
		LoadActor(THEME:GetPathG("_join","icons/single_icon"))..{
		...
		LoadActor(THEME:GetPathG("_join","icons/styleglow"))..{
			...
			GainFocusCommand=function(self) self:accelerate(0.1):diffusealpha(1):x(SCREEN_LEFT+65):sleep(.07):linear(.2):diffusealpha(0) end;
