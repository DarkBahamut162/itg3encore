...
if not isOutFoxV() then
	if isOutFox(20201000) then
		...
		elseif PREFSMAN:PreferenceExists("ImageCache") then
			bannerForced = PREFSMAN:GetPreference("ImageCache") == "ImageCacheMode_Off"
		elseif PREFSMAN:PreferenceExists("CacheImages") then
			bannerForced = PREFSMAN:GetPreference("CacheImages") == "ImageCacheMode_Off"
		end
	end
	if PREFSMAN:PreferenceExists("ShowBanners") and not tobool(PREFSMAN:GetPreference("ShowBanners")) then
		...
		elseif PREFSMAN:PreferenceExists("CacheImages") then
			check = PREFSMAN:GetPreference("CacheImages") == "ImageCacheMode_"..full
		end
...
function cacheStep(Song,Step)
	...
	local _,Hash = SMParser(Step,true)
	...
	if firstBeat then
		...
		if Hash then list["GrooveStatsHash"] = Hash end
		...
		if getenv("cacheing") then
			...
			if Hash then LoadModule("Config.Save.lua")("GrooveStatsHash",Hash,file) end
			if stepType[2] == "Bm" then
				if #noteCounter == 7 or #noteCounter == 14 then
...
function cacheStepSM(Song,Step)
	...
	local chart,Hash = SMParser(Step,true)
	local beat = 0
