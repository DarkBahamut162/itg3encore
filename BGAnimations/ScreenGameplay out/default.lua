return Def.ActorFrame{
	Def.ActorFrame{
		...
		Def.Quad{
			...
			StartTransitioningCommand=function(self) self:playcommand("Check"):diffusealpha(0):linear(0.3):diffusealpha(1) end,
		...
		Def.ActorFrame{
			...
			Def.Sprite {
				...
				StartTransitioningCommand=function(self) self:playcommand("Check"):diffusealpha(0):sleep(0.6):decelerate(0.3):diffusealpha(1):addx(-100) end,
				CheckCommand=function(self) if AnyPlayerFullComboed() then if isEtterna("0.65") then self:sleep(3) else self:hibernate(3) end end end
			},
			Def.Sprite {
	...
	Def.ActorFrame{
		...
		Def.Sprite {
			...
			InitCommand=function(self) self:CenterX():y(SCREEN_CENTER_Y-75*WideScreenDiff()):zoom(0.6*WideScreenDiff()):diffusealpha(0):addy(-30) end,
			ForceFailMessageCommand=function(self) self:stoptweening():sleep(0.1):linear(0.3):diffusealpha(1):addy(30) end
		},
		Def.ActorFrame{
			Name="NormalFail",
			...
			Def.Sprite {
				...
				ForceFailMessageCommand=function(self) self:stoptweening():sleep(0.4):decelerate(0.7):addx(500):diffusealpha(1) end
			}
		},
		Def.ActorFrame{
			Name="VertexFail",
			...
			Def.Sprite {
				...
				ForceFailMessageCommand=function(self) self:stoptweening():sleep(0.7):decelerate(0.75):cropright(0):cropleft(0):diffusealpha(1) end
			},
			Def.Sprite {
				...
				ForceFailMessageCommand=function(self) self:stoptweening():sleep(0.55):accelerate(0.15):cropleft(0):cropright(0):decelerate(0.725):addx(-SCREEN_WIDTH/2):linear(0.15):diffusealpha(0) end
			},
			Def.Sprite {
				Texture = THEME:GetPathG("horiz-line","short"),
