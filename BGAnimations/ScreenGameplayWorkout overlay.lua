	...
	Def.ActorFrame{
		...
		Def.RollingNumbers{
			...
			InitCommand=function(self) self:Load("RollingNumbersCalories"):targetnumber(WorkoutGetTotalCaloriesEvaluation(PLAYER_1))
				:x(THEME:GetMetric("ScreenGameplay","ScoreP1X")):y(THEME:GetMetric("ScreenGameplay","ScoreP1Y")+12)
				:shadowlength(2):diffuse(PlayerColor(PLAYER_1)):addy(-100) end,
		...
		Def.ScoreDisplayAliveTime{
			Font="_r bold numbers",
			PlayerNumber=PLAYER_1,
			InitCommand=function(self) self:x(THEME:GetMetric("ScreenGameplay","ScoreP1X")):y(SCREEN_BOTTOM-38)
				:shadowlength(2):zoom(0.85):diffuse(PlayerColor(PLAYER_1)):addy(100) end,
			OnCommand=function(self) self:sleep(0.5):decelerate(0.8):addy(-100) end,
			OffCommand=function(self) self:accelerate(0.8):addy(100) end
		},
		Def.BitmapText{
			...
			Text="Goal Complete!",
			InitCommand=function(self) self:x(THEME:GetMetric("ScreenGameplay","ScoreP1X")):y(SCREEN_BOTTOM-38-24)
				:zoom(0.6):diffuseshift():effectcolor1(0.5,0.5,0.5,1):diffusealpha(0):addy(100)
			...
			GoalCompleteP1Command=function(self) if not completeGoal[PLAYER_1] then self:zoom(2):linear(0.5):zoom(0.7):diffusealpha(1) completeGoal[PLAYER_1] = true end end
		}
	},
	Def.ActorFrame{
		...
		Def.RollingNumbers{
			...
			InitCommand=function(self) self:Load("RollingNumbersCalories"):targetnumber(WorkoutGetTotalCaloriesEvaluation(PLAYER_2))
				...
				:shadowlength(2):diffuse(PlayerColor(PLAYER_2)):addy(-100) end,
			OnCommand=function(self) self:sleep(0.5):decelerate(0.8):addy(100) end,
			OffCommand=function(self) self:accelerate(0.8):addy(-100) end,
			StepMessageCommand=function(self,params)
		...
		Def.ScoreDisplayAliveTime{
			...
			InitCommand=function(self) self:x(THEME:GetMetric("ScreenGameplay","ScoreP2X")):y(SCREEN_BOTTOM-38)
				:shadowlength(2):zoom(0.85):diffuse(PlayerColor(PLAYER_2)):addy(100) end,
			OnCommand=function(self) self:sleep(0.5):decelerate(0.8):addy(-100) end,
			OffCommand=function(self) self:accelerate(0.8):addy(100) end
		},
		Def.BitmapText{
			...
			GoalCompleteP2Command=function(self) if not completeGoal[PLAYER_2] then self:zoom(2):linear(0.5):zoom(0.7):diffusealpha(1) completeGoal[PLAYER_2] = true end end
		}
	},
