return Def.ActorFrame{
	Def.ActorFrame{
		StartTransitioningCommand=function(self)
			if not (getenv("Restarting") or getenv("Practicing")) then
				self:diffusealpha(0):RunCommandsOnChildren(function(self) self:sleep(0.01):stoptweening():stopeffect() end)
			end
		end,
		Def.Quad{
	...
	Def.ActorFrame{
		StartTransitioningCommand=function(self)
			if getenv("Restarting") or getenv("Practicing") then
				self:diffusealpha(0):RunCommandsOnChildren(function(self) self:sleep(0.01):stoptweening():stopeffect() end)
			end
		end,
		Def.Quad{
		...
		Def.ActorFrame{
			StartTransitioningCommand=function(self)
				if getenv("Restarting") or getenv("Practicing") then
					self:RunCommandsOnChildren(function(self) self:sleep(0.01):stoptweening():stopeffect() end)
				end
			end,
			Def.Sprite {
