...
if scoreType == 1 or scoreType == 4 or scoreType == 5 or scoreType == 6 or scoreType == 7 then dif = 4 elseif scoreType == 2 or scoreType == 8 then dif = 10 elseif scoreType == 3 then dif = 5 end
...
return Def.ActorFrame{
	OnCommand=function(self)
		...
		if scoreDirection == 2 then
			...
			elseif scoreType == 2 or scoreType == 8 then
				displayScore = 10000
	...
	Def.BitmapText {
		...
		InitCommand=function(self)
			self:visible(not getenv("HideScore"..pname(player))):diffuse(PlayerColor(nil)):x(math.floor(scale(player == PLAYER_1 and 0.25 or 0.75,0,1,SCREEN_LEFT,SCREEN_RIGHT))):zoom(0.5*WideScreenDiff()):valign(0)
		...
		JudgmentMessageCommand=function(self,param)
			...
			if scoreType == 4 or scoreType == 5 or scoreType == 6 or scoreType == 7 then update = true end
		...
		W0MessageCommand=function(self,param)
			if param.Player == player then
				...
				W1Count = param.W1
				WXCount = param.WX
				W0Percent = scale(W0Total/(W0Total+(WXCount-W0Count)),0.5,1.0,0.9,1.0)
			end
		end,
		RedrawScoreCommand=function(self)
			...
			elseif scoreType == 2 then
				if scoreDirection == 1 then
					output = animateScore(DP(player)*10000*W0Percent,displayScore)/100
				else
					output = animateScore((DPMax(player)-(DPCurMax(player)-DPCur(player)))/DPMax(player)*10000*W0Percent,displayScore)/100
				end
				self:settextf("%1.2f%%",output) -- PERCENT
			elseif scoreType == 3 then
				if scoreDirection == 1 then
					output = animateScore(math.round(DPCur(player)*W0Percent),displayScore)
				else
					output = animateScore(math.round((DPMax(player)-(DPCurMax(player)-DPCur(player)))*W0Percent),displayScore)
				end
			...
			elseif scoreType == 8 then
				if scoreDirection == 1 then
					...
					output = animateScore((totalwifescore-(maxwifescore-curwifescore))/totalwifescore*10000*W0Percent,displayScore)/100
				end
				self:settextf("%1.2f%%",output) -- WIFE3
			end
			MESSAGEMAN:Broadcast("FAScore",{PLAYER=player,TYPE=(scoreType==2 or scoreType==8) and "Percent" or "Score",SCORE=output})
		end,
		OffCommand=function(self)
			if scoreDirection == 2 then scoreDirection = 1 self:queuecommand("RedrawScore") end
			if scoreType == 8 then setenv("WIFE3FA"..pname(player),curwifescore/totalwifescore*W0Percent) end
		end
	}
}