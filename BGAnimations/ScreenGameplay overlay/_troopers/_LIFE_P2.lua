return Def.ActorFrame{
	Def.ActorFrame{
		InitCommand=function(self) self:x(isDouble() and 0 or -77):y(36):visible(GAMESTATE:IsPlayerEnabled(PLAYER_2) and (getenv("IIDXPercent"..pname(PLAYER_2)) or false)) end,
		Def.Sprite {
			...
			Def.Sprite {
				...
				LifeChangedMessageCommand=function(self,param)
					if param.Player == PLAYER_2 then
						local life = param.LifeMeter:GetLife()
						life = math.round(life/2,2)*2
						self:cropright(1 - life + 0.02):cropleft(life - 0.06)
					end
				end
			}
		}
