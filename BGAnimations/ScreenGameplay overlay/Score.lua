...
return Def.ActorFrame{
	...
	Def.BitmapText {
		...
		InitCommand=function(self)
			...
			if scoreType == 1 then self:zoom(WideScreenDiff_(16/10)) else self:zoom(WideScreenDiff()) end
		...
		RedrawScoreCommand=function(self)
			...
			elseif scoreType == 5 then
				...
				if scoreDirection == 1 then
					...
					score = (math.floor(score-sub) - (w2 + w3 + w4))*10
					output = animateScore(score,displayScore)
				else
					local w5 = stats:GetTapNoteScores('TapNoteScore_W5')
					local ms = stats:GetTapNoteScores('TapNoteScore_Miss')
					...
					output = animateScore(maxScore-(math.ceil(curMaxScore-score+subScore)+w2+w3+w4)*10,displayScore)
				end
			...
			elseif scoreType == 7 then
				...
				local hd = stats:GetHoldNoteScores('HoldNoteScore_Held')
				if scoreDirection == 1 then
					score = (w1+hd + w2*(0.7) + w3*(0.4)) * 100000 / stepSize
					output = animateScore(math.floor(score),displayScore)
				else
					local w4 = stats:GetTapNoteScores('TapNoteScore_W4')
					local w5 = stats:GetTapNoteScores('TapNoteScore_W5')
			...
			MESSAGEMAN:Broadcast("Score",{PLAYER=player,TYPE=(scoreType==2 or scoreType==8) and "Percent" or "Score",SCORE=output})
		end,
