return Def.ActorFrame{
	...
	Def.ActorFrame{
		...
		Def.Sprite {
			...
			InitCommand=function(self) self:zoomy(-1) end
		},
		Def.BitmapText {
			File = "_iidx/Percent Blue",
			InitCommand=function(self) self:y(2):x(-1):maxwidth(30) end,
	...
	Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			Def.Sprite {
				Texture = "../life/"..IIDXLifeBar(PLAYER_1),
				InitCommand=function(self) self:diffuseblink():effectperiod(0.16):effectcolor1(color("1,1,1,1")):effectcolor2(color(".3,.3,.3,1")):x(-7):y(-1):visible((GAMESTATE:IsPlayerEnabled(PLAYER_1))) end,
				LifeChangedMessageCommand=function(self,param)
					if param.Player == PLAYER_1 then
						local life = param.LifeMeter:GetLife()
						life = math.round(life/2,2)*2
