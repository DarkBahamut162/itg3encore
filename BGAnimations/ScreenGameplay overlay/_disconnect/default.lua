...
return Def.ActorFrame{
	...
	Def.ActorFrame{
		...
		LoadFont("_r bold 30px")..{
			InitCommand=function(self) self:visible(not isOutFox()):CenterX():y(SCREEN_TOP+24*WideScreenDiff()):maxwidth(420):diffusebottomedge(color("#dedede")) end,
		...
		LoadActor("_base normal")..{ InitCommand=function(self) self:CenterX():y(SCREEN_TOP+19*WideScreenDiff()):zoom(WideScreenDiff()) end },
		LoadActor("_base bpm")..{ InitCommand=function(self) self:CenterX():y(SCREEN_TOP+59*WideScreenDiff()):zoom(WideScreenDiff()):visible(not isRave()) end },
		...
		LoadActor("_neons bottom")..{
			InitCommand=function(self) self:CenterX():y(SCREEN_TOP+39*WideScreenDiff()):zoom(WideScreenDiff()):blend(Blend.Add):visible(not isRave()) end,
			OnCommand=function(self) self:effectclock('beat'):diffuseramp():effectcolor1(color("#007892")):effectcolor2(color("#00EAFF")):effectperiod(0.5):effect_hold_at_full(0.5):diffusealpha(0):linear(0.4):diffusealpha(1) end
		},
		LoadActor("_neons bottom")..{
			InitCommand=function(self) self:CenterX():y(SCREEN_TOP+39*WideScreenDiff()):zoom(WideScreenDiff()):visible(not isRave()) end,
			OnCommand=function(self) self:effectclock('beat'):diffuseramp():effectcolor1(color("#FFFFFF00")):effectcolor2(color("#82f5ff")):effectperiod(0.5):effect_hold_at_full(0.5):diffusealpha(0):linear(0.4):diffusealpha(1) end
		}
	},
	Def.ActorFrame{
