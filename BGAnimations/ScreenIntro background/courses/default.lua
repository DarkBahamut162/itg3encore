return Def.ActorFrame{
	Def.ActorFrame{
		...
		OnCommand=function(self) self:sleep(21):linear(6):y(SCREEN_CENTER_Y+(SCREEN_HEIGHT/12)*WideScreenDiff()) end,
		LoadFont("_v credit")..{
			Text=GetRandomCourseNames(math.ceil(23/WideScreenDiff())),
	...
	Def.ActorFrame{
		...
		LoadActor("orange "..(isFinal() and "final" or "normal"))..{
			InitCommand=function(self) self:x(SCREEN_RIGHT+SCREEN_WIDTH*2):zoomto(SCREEN_WIDTH*1.5,100) end,
			OnCommand=function(self) self:sleep(21):linear(0.5):fadeleft(0.3):faderight(0.3):x(50):linear(4):addx(-100*WideScreenDiff()):accelerate(0.5):addx(-SCREEN_WIDTH*2) end
		},
		LoadActor("courses "..(isFinal() and "final" or "normal"))..{
	...
	Def.ActorFrame{
		...
		LoadActor(THEME:GetPathB("ScreenIntro","background/swoosh "..(isFinal() and "final" or "normal")))..{
			...
			OnCommand=function(self) self:sleep(21):linear(0.5):cropright(0):faderight(0):sleep(4):linear(0.5):diffusealpha(0.0):addx(50*WideScreenDiff()):cropleft(1):fadeleft(1) end
		},
		Def.ActorFrame{
			OnCommand=function(self) self:sleep(21):linear(6):addx(-20*WideScreenDiff()) end,
			LoadActor("70 "..(isFinal() and "final" or "normal"))..{
