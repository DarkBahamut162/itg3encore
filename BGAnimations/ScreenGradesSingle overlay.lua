...
for i=2,7 do
	local nums = Def.ActorFrame{
		Name="NumbersTier"..i,
		InitCommand=function(self) self:y(scale(i,1,7,SCREEN_CENTER_Y-130,SCREEN_CENTER_Y+130)) end
	}
	for s=1,10 do
		local score = LoadFont("_futurist metalic")..{
			...
			BeginCommand=function(self)
				...
				if i > 5 then
					self:settext(prof:GetTotalTrailsWithTopGrade('StepsType_Dance_Single',diffs[i],tier))
				else
					self:settext(prof:GetTotalStepsWithTopGrade('StepsType_Dance_Single',diffs[i],tier))
				end
			end
		}
		nums[#nums+1] = score
