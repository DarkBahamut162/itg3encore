return Def.ActorFrame{
	...
	Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			LoadActor("p1win text "..(isFinal() and "final" or "normal"))..{
			...
			LoadActor("p1win glow "..(isFinal() and "final" or "normal"))..{
		...
		Def.ActorFrame{
			...
			LoadActor("p2win text "..(isFinal() and "final" or "normal"))..{
				OnCommand=function(self) self:cropright(1.3):faderight(0.1):linear(0.7):cropright(-0.3):sleep(2.2):linear(0.3):diffuse(0,0,0,0) end
			},
			LoadActor("p2win glow "..(isFinal() and "final" or "normal"))..{
				OnCommand=function(self) self:cropleft(-0.3):cropright(1):faderight(0.1):fadeleft(0.1):linear(0.7):cropleft(1):cropright(-0.3) end
			}
		},
		Def.ActorFrame{
			Condition=not GAMESTATE:IsWinner(PLAYER_1) and not GAMESTATE:IsWinner(PLAYER_2),
			LoadActor("draw text "..(isFinal() and "final" or "normal"))..{
				OnCommand=function(self) self:cropright(1.3):faderight(0.1):linear(0.7):cropright(-0.3):sleep(2.2):linear(0.3):diffuse(0,0,0,0) end
			},
			LoadActor("draw glow "..(isFinal() and "final" or "normal"))..{
				OnCommand=function(self) self:cropleft(-0.3):cropright(1):faderight(0.1):fadeleft(0.1):linear(0.7):cropleft(1):cropright(-0.3) end
			}
		}
	}
