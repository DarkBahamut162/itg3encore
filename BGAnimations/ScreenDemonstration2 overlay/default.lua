...
return Def.ActorFrame{
	...
	LoadFont("_v tutorial")..{
		Text="Colored Arrows\nscroll from\nlow to high.",
		InitCommand=function(self) self:x(posX_now):CenterY():addx(SCREEN_WIDTH/2):zoom(1) end,
	...
	LoadFont("_v tutorial")..{
		Text="Step when a\nColored Arrow\noverlaps the\nTarget Arrows\nat the top.",
		InitCommand=function(self) self:x(posX_now):CenterY():addx(SCREEN_WIDTH):zoom(1) end,
	...
	LoadFont("_v tutorial")..{
		Text="For arrows\nfacing Left,\nstep on the\nLeft Panel.",
		InitCommand=function(self) self:x(posX_now):y(SCREEN_CENTER_Y-80):addx(SCREEN_WIDTH/2):zoom(1) end,
		OnCommand=function(self) self:sleep(29):decelerate(0.5):addx(-SCREEN_WIDTH/2):sleep(5):linear(0.3):diffusealpha(0) end
	},
	LoadActor("arrow")..{
		InitCommand=function(self) self:x(posX_now-66):y(SCREEN_CENTER_Y+80):rotationz(0):glowblink():effectperiod(0.5):diffusealpha(0) end,
		OnCommand=function(self) self:sleep(30):linear(0.3):diffusealpha(1):sleep(4):linear(0.3):diffusealpha(0) end
	},
	Def.ActorFrame{
		Name="PlatformLeft",
		InitCommand=function(self) self:x(posX_now+33):y(SCREEN_CENTER_Y+80):zoom(0.8):rotationx(-20):fov(45):vanishpoint(SCREEN_CENTER_X+210,SCREEN_CENTER_Y+80) end,
	...
	LoadActor("arrow")..{
		InitCommand=function(self) self:x(posX_now-66):y(SCREEN_CENTER_Y+80):rotationz(90):glowblink():effectperiod(0.5):diffusealpha(0) end,
