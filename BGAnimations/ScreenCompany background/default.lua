if isOutFox(20200900) then
	...
	if beginnerHelper == true then PREFSMAN:SetPreference("ShowBeginnerHelper",false) end
	local beginnerHelperBackgrounds = PREFSMAN:PreferenceExists("EnableBeginnerHelperBackgrounds") and tobool(PREFSMAN:GetPreference("EnableBeginnerHelperBackgrounds")) or nil
	if beginnerHelperBackgrounds == false then PREFSMAN:SetPreference("EnableBeginnerHelperBackgrounds",true) end
end
...
if isITGmania() and NETWORK:IsUrlAllowed(url) then
	NETWORK:HttpRequest{
		url=url,
