local t = Def.ActorFrame{
	Def.ActorFrame{
		InitCommand=function(self) self:y(SCREEN_CENTER_Y-159) end;
		LoadActor(THEME:GetPathG("_name","badge"),PLAYER_1)..{
			InitCommand=function(self) self:x(SCREEN_CENTER_X+52):addx(SCREEN_WIDTH*3/4):player(PLAYER_1) end;
		...
		LoadActor(THEME:GetPathG("_name","badge"),PLAYER_2)..{
			InitCommand=function(self) self:x(SCREEN_CENTER_X+220):addx(SCREEN_WIDTH*3/4):player(PLAYER_2) end;
	...
	LoadActor(THEME:GetPathB("ScreenOptions","overlay/_frame"))..{
		...
		OnCommand=function(self) self:diffuseblink():effectcolor1(color("0.8,0.8,0.8,.85")):effectoffset(.6):effectperiod(0.4):effectoffset(0.2):effectclock('beat'):diffusealpha(0):zoom(.7):bounceend(0.3):diffusealpha(1):zoom(1) end;
