...
if getenv("SetPacemaker"..pname(pn)) == 18 then
	...
	for i=1,#song do
		...
		for pms in ivalues(PaceMaker[pn][category][math.floor(SPS)] or {}) do
		...
		if #(PaceMaker[pn][category][math.floor(SPS)] or {}) <= 1 then target = 0.5 end
	end
else
	...
	Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			Def.Sprite {
				...
				InitCommand=function(self)
					self:y(-tmax*barHeight+barHeight/2):valign(0):zoomx(1.25):zoomy(0.5):diffusealpha(0.5):fadeleft(0.25):faderight(0.25):diffuse(color("#FF0000")):diffuseramp():effectcolor1(color("#FF000080")):effectcolor2(color("#FF0000FF")):effectperiod(0.5):effect_hold_at_full(0.5):effectclock('beat')
				end,
