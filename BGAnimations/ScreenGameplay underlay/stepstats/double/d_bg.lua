...
return Def.ActorFrame{
	...
	Def.ActorFrame{
		...
		Def.Sprite {
			...
			InitCommand=function(self)
				self:x(-barWidth[bgNum]/2):y(-topscore:GetPercentDP()*268+164):valign(0):zoomx(0.08/3*2):zoomy(0.5):diffusealpha(1):fadeleft(0.25):faderight(0.25):diffuse(color("#00FF00")):diffuseramp():effectcolor1(color("#00FF0080")):effectcolor2(color("#00FF00FF")):effectperiod(0.5):effect_hold_at_full(0.5):effectclock('beat')
			end,
			JudgmentMessageCommand=function(self,param) if param.Player == pn and topscore ~= nil and self:GetDiffuseAlpha() == 1 then self:queuecommand("Update") end end,
			UpdateCommand=function(self)
				local pss = STATSMAN:GetCurStageStats():GetPlayerStageStats(pn)
