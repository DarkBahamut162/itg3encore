if isEtterna("0.60") and PREFSMAN:GetPreference("AutoConnectMultiplayer") == 1 then
	...
	if not KeysChecked and ThemePrefs.Get("KeyboardEnabled") and FILEMAN:DoesFileExist("Save/Keymaps.ini") then
		...
		RefreshCommand=function(self)
			if not isTopScreen("ScreenLogo") then
				...
				if (StepsTypeSingle() or StepsTypeDouble()) and (IsGame("dance") or isOutFoxV() or not isOutFox()) then
					...
					if isTopScreen('ScreenTitleJoin') and (IsAutoPlayMode(true) and IsAutoStyle()) then output = addToOutput(output,IsAutoPlayMode(),"\n") end
					if isTopScreen('ScreenTitleJoin') and (IsAutoPlayMode(true) and IsAutoStyle()) or (isTopScreen('ScreenTitleMenu') and IsAutoStyle()) then
